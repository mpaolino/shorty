...
class UASparser:
    ...
    def __init__(self, cache_dir=None):
        ...
        """
        self.cache_dir = cache_dir and cache_dir or \
    ...
    def parse(self, useragent, entire_url=''):
        ...
        Args:
            ...
            entire_url: String, write the key labels which you want to get an
                        entire url split by comma, expected 'ua_icon' or
                        'os_icon'.
        """
        ret = {'typ': 'unknown',
               'ua_family': 'unknown',
            ...
            if test[0] == useragent:
                ...
                for i in range(1, len(test) + 1):  # Fill out the structure.
                    if i < 6:
                        ...
                        ret[ua_index[i - 1]] = ('ua_icon' in entire_url and \
                                                self.ua_img_url or "%s") % (
                                                    test[i])
                    elif i == 7:
                        if test[7]:  # OS detail.
                        ...
                        ret[ua_index[i - 1]] = ''.join([self.info_url,
                                                        test[i]])
        ...
        for index in data['browser_reg']['order']:
            ...
            # All regular expression should be in python format.
        ...
        # Get browser detail.
        if id_browser:
            _index = ['ua_family', 'ua_url', 'ua_company', 'ua_company_url',
                      'ua_icon', 'ua_info_url']
            try:
                #Any better method to figure this out?
                if id_browser in data['browser']:
                    for i in range(1, len(data['browser'][id_browser]) + 1):
                        if i <= 4:
                            ret[_index[i - 1]] = data['browser'][id_browser][i]
                        elif i == 5:
                            ret[_index[i - 1]] = ('ua_icon' in entire_url and \
                                                  self.ua_img_url or "%s") % (
                                                      data['browser'][id_browser][i])
                        else:
                            ret[_index[i - 1]] = "".join(
                                [self.info_url,
                                 data['browser'][id_browser][i]])
            ...
            try:
                ...
                ret['ua_name'] = "%s %s" % (data['browser'][id_browser][1],
                                            info)
            except:
                pass
            ...
            try:
                ...
                for i in range(0, len(data['os'][os_id])):
                    if i < 5:
                        ...
                        ret[os_index[i]] = ('os_icon' in entire_url and \
                                            self.os_img_url or "%s") % (
                                                data['os'][os_id][i])
                return ret
            except:
                pass
    ...
    def _parseIniFile(self, ini_file):
        """
        Parse an ini ini_file into a dictionary structure
        """
        data = {}
        ...
        for line in ini_file.split("\n"):
            ...
            if option:  # Do something for option.
                ...
                section = section_pat.findall(line)  # Do someth for section.
                if section:
